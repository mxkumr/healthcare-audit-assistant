sap.ui.define([
  ...
  ControllerExtension.extend('com.medicare.31riskdynamics.controller.ListReportExt', {
    metadata: {
      methods: {
    ...
    override: {
      onBeforeRendering() {
        const dataModel = new JSONModel({
          Query: 'Show specialties with the highest average cost per patient for a bar chart.'
        });
    ...
    openDiagram() {
      ...
      if (!this._oDiagramDialog) {
        this._oDiagramDialog = Fragment.load({
          id: oView.getId(),
          name: 'com.medicare.31riskdynamics.ext.fragment.UploadDiagram',
          controller: this
        }).then((oDialog) => {
