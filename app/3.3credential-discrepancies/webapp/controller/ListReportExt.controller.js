sap.ui.define([
  ...
  ControllerExtension.extend('com.medicare.33credentialdiscrepancies.controller.ListReportExt', {
    metadata: {
      methods: {
    ...
    override: {
      onBeforeRendering() {
        const dataModel = new JSONModel({
          Query: 'Show credential groups with the highest charge padding amount for a bar chart.'
        });
    ...
    openDiagram() {
      ...
      if (!this._oDiagramDialog) {
        this._oDiagramDialog = Fragment.load({
          id: oView.getId(),
          name: 'com.medicare.33credentialdiscrepancies.ext.fragment.UploadDiagram',
          controller: this
        }).then((oDialog) => {
