sap.ui.define([
  ...
  ControllerExtension.extend('com.medicare.riskanalysis.controller.ListReportExt', {
    metadata: {
      methods: {
    ...
    override: {
      onBeforeRendering() {
        const dataModel = new JSONModel({
          Query: 'Show risk bands with the highest total Medicare paid for a bar chart.'
        });
    ...
    openDiagram() {
      ...
      if (!this._oDiagramDialog) {
        this._oDiagramDialog = Fragment.load({
          id: oView.getId(),
          name: 'com.medicare.riskanalysis.ext.fragment.UploadDiagram',
          controller: this
        }).then((oDialog) => {
