...
| `ensure-mbt.js` fails in `gen/srv` | Fixed: removed `postinstall` hook (CDS copies it into `gen/srv` where the script does not exist). Use `npm run ensure-mbt` at project root only. |
| `mbt/unpacked_bin/mbt: not found` | Run `npm run ensure-mbt` after `npm ci` (also runs in MTA `before-all`). Fallback: `npm install -g mbt && npm run ensure-mbt` |
